...
function moduleBody(mod) {
  ...
  if (mod.type === 'todo') { const done = (mod.items || []).filter(i => i.done).length; return `<div class="progress-line"><span>${done}/${(mod.items || []).length} 完成</span><b><i style="width:${(mod.items || []).length ? done / mod.items.length * 100 : 0}%"></i></b></div><div class="todo-list">${(mod.items || []).map((i, n) => `<label><input type="checkbox" data-todo="${mod.id}" data-index="${n}" ${i.done ? 'checked' : ''}><span class="${i.done ? 'done' : ''}">${esc(i.text)}</span>${editMode ? `<span class="inline-actions"><button data-action="edit-todo-item" data-id="${mod.id}" data-index="${n}" title="编辑任务">✎</button><button data-action="delete-todo-item" data-id="${mod.id}" data-index="${n}" title="删除任务">×</button></span>` : ''}</label>`).join('')}</div><button class="text-btn" data-action="add-todo" data-id="${mod.id}">+ 添加任务</button>`; }
  if (mod.type === 'table') return `<div class="data-table"><div class="table-row table-header ${editMode ? 'with-actions' : ''}"><span>名称</span><span>状态</span><span>时间</span>${editMode ? '<span>操作</span>' : ''}</div>${(mod.rows || []).map((r, n) => `<div class="table-row ${editMode ? 'with-actions' : ''}" data-table-row="${mod.id}" data-row="${n}"><span>${esc(r[0])}</span><span><em class="status ${r[1] === '已完成' ? 'complete' : r[1] === '进行中' ? 'active' : ''}">${esc(r[1])}</em></span><span>${esc(r[2])}</span>${editMode ? `<span class="inline-actions"><button data-action="edit-row" data-id="${mod.id}" data-index="${n}" title="编辑行">✎</button><button data-action="delete-row" data-id="${mod.id}" data-index="${n}" title="删除行">×</button></span>` : ''}</div>`).join('')}</div><button class="text-btn" data-action="add-row" data-id="${mod.id}">+ 添加一行</button>`;
  if (mod.type === 'stats') { const metrics = mod.metrics || structuredClone(defaultMetrics); return `<div class="stat-grid">${metrics.map((metric, n) => `<div><strong>${esc(metric.value)}</strong><span>${esc(metric.label)}</span>${editMode ? `<span class="inline-actions"><button data-action="edit-metric" data-id="${mod.id}" data-index="${n}" title="编辑指标">✎</button><button data-action="delete-metric" data-id="${mod.id}" data-index="${n}" title="删除指标">×</button></span>` : ''}</div>`).join('')}</div><div class="bars"><i style="height:35%"></i><i style="height:55%"></i><i style="height:42%"></i><i style="height:78%"></i><i style="height:62%"></i><i style="height:90%"></i><i style="height:72%"></i></div><div class="chart-labels"><span>一</span><span>二</span><span>三</span><span>四</span><span>五</span><span>六</span><span>日</span></div>${editMode ? '<button class="text-btn" data-action="add-metric" data-id="' + mod.id + '">+ 添加指标</button>' : ''}`; }
  ...
  return `<div class="custom-body"><span class="custom-tag">CUSTOM</span><p>${nl(mod.content || '这是一个可自由组合的模块。')}</p>${mod.fields?.length ? `<div class="field-list">${mod.fields.map((f, n) => `<span><b>${esc(f.label)}</b>${esc(f.value || '未设置')}${editMode ? `<span class="inline-actions"><button data-action="edit-field" data-id="${mod.id}" data-index="${n}" title="编辑字段">✎</button><button data-action="delete-field" data-id="${mod.id}" data-index="${n}" title="删除字段">×</button></span>` : ''}</span>`).join('')}</div>` : ''}${editMode ? `<button class="text-btn" data-action="add-field" data-id="${mod.id}">+ 添加字段</button>` : ''}</div>`;
}
