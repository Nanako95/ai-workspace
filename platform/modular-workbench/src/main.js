...
function normalizeState(raw) {
  ...
  next.pages = (raw?.pages || seed.pages).map(page => {
    ...
    return { ...page, activeCategory: page.activeCategory || 'all', categories, modules: (page.modules || []).map(mod => ({ ...mod, categoryId: categoryIds.has(mod.categoryId) ? mod.categoryId : categoryForType(mod.type, categories) })) };
  });
